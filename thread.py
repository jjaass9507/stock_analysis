...
def _get_analyzed_codes_by_date(engine, date):
    """回傳指定日期已存入 DB 的股票代碼集合，供增量更新跳過使用。"""
    with engine.connect() as conn:
        result = conn.execute(
...
def crawler():
    ...
    for url, market in [
        ...
        try:
            ...
            pass
    if not all_stocks:
        print("isin.twse.com.tw 連線失敗，改用 TWSE/TPEx OpenAPI...")
        all_stocks = cr.get_stock_list_openapi()
    if not all_stocks:
        raise RuntimeError("無法取得台股代碼清單（isin.twse.com.tw 及 TWSE/TPEx OpenAPI 皆失敗）")
